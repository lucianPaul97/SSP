...
public class SudokuTable {
    ...
    public SudokuTable(UUID id, int size) {
        mId = id;
        this.size = size;
    ...
    public void setData(int index, int value) {
        ...
        Log.v("SudokuTable-checkEntry", "value " + value + " entered at index [" + x + "," + y + "]");
    }
    ...
    public void deleteSingleData(int index) {
        ...
        for (int i = entries.size()-1; i >= 0; i--) {
            if (entries.get(i).x == x && entries.get(i).y == y) {
                Log.v("SudokuTable-deleteData", "removed " + data[x][y] + " at [" + x + "," + y + "]   entry size: " + entries.size());
                entries.remove(i);
                break;
            }
        }
    ...
    public boolean solvePuzzle() {
        ...
        if(solvable && ACOAlgorithm.solve(solvedData)) { //recursive method that solves puzzle in solvedData
            hasBeenSolved = true;
            return true;
        }
        else {
